...
If you want to enforce a commit message with the version tag, you can do so by adding `force_commit_message` like this:

  Rake::VersionTask.new do |task|
    task.force_commit_message = true
  end

Then you is required to add a message parameter, like

	rails version:bump['This is the commit message']
